...
import { MessageCircle, ShieldCheck, Hammer, Sparkles, Ruler, ChevronRight, Truck } from "lucide-react";
import { categoriesData, productsData } from "@/lib/productsData";
...
export default function ProductDetailPage() {
  ...
  const relatedProducts = productsData
    .filter((p) => p.subcategorySlug === subcategorySlug && p.slug !== productSlug)
    .slice(0, 4);
  ...
  const waUrl = `https://wa.me/919074477358?text=${waMessage}`;
    ...
    <div className="flex flex-col w-full min-h-screen bg-white">
      ...
      <section className="py-10 md:py-16 max-w-[1600px] mx-auto px-4 lg:px-8 xl:px-16 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-16">
          ...
          {/* Gallery */}
          <div className="lg:col-span-8 flex flex-col-reverse lg:flex-row gap-4 lg:sticky lg:top-28 self-start">
            ...
            {/* Main Image */}
            <div className="relative flex-1 aspect-[4/3] w-full overflow-hidden bg-[#F4ECE1] rounded-sm">
              <Image
                src={activeImage}
                alt={product.name}
                fill
          ...
          <div className="lg:col-span-4 flex flex-col justify-start">
            ...
            <p className="text-[#2A1C14]/70 font-sans text-sm leading-relaxed mb-8">
              {product.description}
            </p>
      ...
      <section className="border-t border-gray-100 py-12">
        <div className="max-w-[1600px] mx-auto px-4 lg:px-8 xl:px-16 grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="flex gap-4">
            <Sparkles className="w-5 h-5 text-[#592915] shrink-0 mt-0.5" />
            <div>
              <h3 className="font-serif text-base text-[#592915] mb-1.5">Genuine Timber</h3>
              <p className="text-xs text-[#2A1C14]/55 leading-relaxed font-sans">
                Hand-selected logs vetted for structural density and grain aesthetics, kiln-dried to perfection.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            <Ruler className="w-5 h-5 text-[#592915] shrink-0 mt-0.5" />
            <div>
              <h3 className="font-serif text-base text-[#592915] mb-1.5">Custom Sizing</h3>
              <p className="text-xs text-[#2A1C14]/55 leading-relaxed font-sans">
                Every dimension can be adapted. Choose carving motifs and storage configurations to suit your space.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            <Hammer className="w-5 h-5 text-[#592915] shrink-0 mt-0.5" />
            <div>
              <h3 className="font-serif text-base text-[#592915] mb-1.5">Heritage Joinery</h3>
              <p className="text-xs text-[#2A1C14]/55 leading-relaxed font-sans">
                Traditional mortise & tenon joints — no screws or nails for the primary structural frame.
              </p>
            </div>
          </div>
        </div>
